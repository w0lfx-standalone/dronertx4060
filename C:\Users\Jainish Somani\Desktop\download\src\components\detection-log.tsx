    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            AI-powered analysis of why this object was flagged.
          </DialogDescription>
        </DialogHeader>
        <div className="py-4">
          <Image
            src={event.frameDataUri}
            alt="Detected object frame"
            width={400}
            height={225}
            className="rounded-md mb-4"
            data-ai-hint="flying object"
          />
          {isLoading ? (
            ...
            <p className="text-sm text-foreground/90">{explanation || event.explanation}</p>
          )}
        </div>
      </DialogContent>
    </Dialog>
  ...
  return (
    <Card className="h-full flex flex-col">
      <CardHeader>
        ...
        <CardDescription>A log of all detected flying objects.</CardDescription>
      </CardHeader>
        ...
        <ScrollArea className="h-full pr-4 -mr-4">
          {events.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-center">
                ...
                <p className="text-sm">Events will appear here when a flying object is detected.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {events.map((event) => (
                <Card key={event.id} className={`overflow-hidden ${event.objectType === 'drone' ? 'border-destructive/50' : ''}`}>
                  <div className="grid grid-cols-3">
                    ...
                    <div className="col-span-2 p-3">
                        <div className="flex items-center justify-between">
                            <p className={`font-semibold text-sm capitalize ${event.objectType === 'drone' ? 'text-destructive' : ''}`}>
                                {event.objectType}
                            </p>
                            <EventIcon objectType={event.objectType} />
                        </div>
